...
{
  const numberTypedArray: number[] = [10, 101, 201, 301]; // ? Array<number>도 가능

  let total = numberTypedArray.reduce((total, value) => value + total, 0);
  console.log(total);

  numberTypedArray.push(+"901", 2, 34, 5, 1, 2, 3); // & 문자는 매개 변수에 할당될 수 없다.

  const stringTypedArray =
    "annotation, inference, intersection, type alias".split(",");
  console.log(stringTypedArray.length);

  stringTypedArray.map((subject) => subject.trim()); // ? trim = trimStart, trimEnd 모두
